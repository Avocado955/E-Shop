...
const Carousel = () => {
  ...
  useEffect(() => {
    getAllProductsWithMatchingField("featured", true).then((data) =>
  ...
  //Classes
  const nextClass = `${styles.slideBtn} ${styles.next}`;
  const prevClass = `${styles.slideBtn} ${styles.prev}`;
    ...
    <div className={styles.carousel}>
      {featuredProducts && (
        <div className={styles.carousel_window}>
          <img
            src={featuredProducts[carouselIndex].imageURL}
            alt=""
            className={styles.img}
          />
          <div className={styles.carousel_window_textSide}>
            <h2>
              {featuredProducts[carouselIndex].colour +
                " " +
                featuredProducts[carouselIndex].name}
            </h2>
            <h3>${featuredProducts[carouselIndex].price} AUD</h3>
            <button className={styles.btn} onClick={goToFeaturedProduct}>
              Product Page
            </button>
          </div>
        </div>
      )}
      <button className={nextClass} onClick={onNextBtn}>
        <span>&#8680;</span>
      </button>
      <button className={prevClass} onClick={onPreviousBtn}>
        <span>&#8678;</span>
      </button>
    </div>
  );
